...
describe("MutantGeneLeftDiagonalFinder Test Suite", () => {
	describe("Success Cases", () => {
		...
		it("containsMutantSequence should Return 1 if a mutant sequence is found in a left diagonal", async () => {
			const genome: Genome = [
					["A", "C", "G", "T", "A"],
					["A", "A", "C", "G", "T"],
					["T", "A", "A", "C", "G"],
				...
				mutantGeneFinder: MutantGeneFinder = new MutantGeneLeftDiagonalFinder();
			const containsMutantSequence = await mutantGeneFinder.findNumberOfMutantSequence(genome);
			expect(containsMutantSequence).toBeGreaterThanOrEqual(1);
		});

		it("containsMutantSequence should Return 0 if a mutant sequence is not found in a left diagonal", async () => {
			const genome: Genome = [
					["A", "C", "G", "T"],
					["C", "G", "T", "A"],
					["G", "T", "A", "C"],
